...
class ariachart {
  ...
  function newsgroup($SERVER, $USER, $PASS, $GROUP=NULL) {
    ...
    if ($GROUP == NULL) {
      ...
      $news = imap_open($SERVER, $USER, $PASS, OP_ANONYMOUS);

      echo 'Listing all alt.binaries.hdtv.* ['. $SERVER .']'.CRLF;
      $group = imap_getmailboxes($news, $SERVER, "alt.binaries.hdtv*");
      ...
      for ($i = $total-100; $i <= $total; $i++) {
        ...
        echo $post->subject .' '. $post->from .CRLF;
      }
...
// print_r($a->getChart('albums'));
 ...
 $a->newsgroup('news.tpg.com.au', 'damiankw', 'maddog', 'alt.binaries.hdtv');
?>