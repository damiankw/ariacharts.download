...
class ariaget {
  ...
  function show_new() {
    $q = mysql_query("SELECT DISTINCT date, artist, title, category.name FROM charts, category WHERE (last = 0) AND (charts.cat_id = category.cat_id) ORDER BY date DESC, name, artist", $this->LINK);
    $c = 0;
  ...
  function chart_store() {
    ...
    $QUERY = mysql_query("SELECT cat_id, chart, name FROM ". $this->SQL['tb_catinfo'], $this->LINK);
    ...
    while ($DATA = mysql_fetch_array($QUERY)) {
      ...
      foreach ($CHART as $KEY => $VALUE) {
        mysql_query("INSERT INTO ". $this->SQL['tb_chartinfo'] ." (date, cat_id, this, last, title, artist) VALUES(UNIX_TIMESTAMP(DATE(NOW())), '". $DATA['cat_id'] ."', '". $VALUE['this'] ."', '". $VALUE['last'] ."', '". $VALUE['title'] ."', '". $VALUE['artist'] ."')", $this->LINK);
      }
